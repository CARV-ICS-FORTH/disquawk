...
\begin{itemize}
\item Before each \verb!monitorexit! and \verb!volatile! store completion
  \begin{itemize}
  \item all \emph{dirty} objects (on this core) must be flushed to the
    memory

    \notebox{ There is no need to empty our software-cache. }
  \item any preceding load or \verb!monitorenter! instructions must
    complete (and be visible by other processors if needed)
  \end{itemize}
\item Before each \verb!monitorenter! and \verb!volatile! load all
  preceding \verb!monitorexit! instructions and \verb!volatile! stores
  must complete.

  \notebox{ That said, what is mostly important, is to make sure any
    pending write-backs from a (globally) previous \texttt{monitorexit}
    have come to completion. The simplest approach here is to make
    \texttt{monitorexit} block until completion of the write-backs,
    however this seems totally inefficient. That raises the need of a
    mechanism that can understand if there are any outstanding
    write-backs. On the other hand in the case of volatile stores
    this, blocking seems reasonable.
  }

\item At \verb!monitorenter! and \verb!volatile! loads we have to wait
  until they complete before we continue with the execution.
  Furthermore, we have to ensure that any cached objects are
  up-to-date.

  \notebox{ The simplest way to achieve this, is to empty our
    software-cache and re-fetch the objects upon request.}

  \notebox{ Pre-fetching data would save a significant amount of time
    as the DMA transfers would overlap with code execution. However,
    for \gvm to be aware of what objects need to be fetched a priori,
    the programmer or even better a static analysis must provide
    appropriate hints. In a data-flow execution model this is even
    easier to achieve, combining the programmer's data-flow
    annotations with a static analysis on the code.}

  \notebox{ A possible improvement here would be to use versioning of
    the cached objects instead of flushing the whole software cache at
    every synchronization point. With versioning a core can simply
    query the \emph{home} DRAM for the latest version of the cached
    objects. Then it can only fetch the remote objects that feature a
    higher version number than the cached data. In this case we would
    probably like to use some dedicated cores (like the ones discussed
    in the next section) that would be responsible for answering such
    queries and even initiate the DMA transfers. The version number
    could be kept in the object's header.}
...
\item All \verb!static final! fields must be initialized before the
  ...
  \notebox{ Squawk should already be taking care of this. If not, a
    simple solution is to use a ready flag per object (probably in its
    header).}
\end{itemize}

\cautionbox{ JNI routine's entry and exit points might need to be synchronized. }
...
\notebox{
  Your IP must be allowed to access formicarium. Please
