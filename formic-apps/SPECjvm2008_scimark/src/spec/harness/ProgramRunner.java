...
public class ProgramRunner extends Thread {
    ...
    private boolean setupBenchmarkMethods() {
        ...
        if (bmResult == null) {
            System.out.println("The benchmark is not setup properly. There is no BenchmarkResult with configuration.");
    ...
    private boolean runBenchmark() {
        ...
        try {
            ...
            System.out.println("--- --- --- --- --- --- --- --- ---");
            System.out.println();
            System.out.println("  Benchmark:   " + bmResult.getLogicalName());
            System.out.println("  Run mode:    " + TestResult.getRunModeDescription(bmResult.getRunMode()));
            System.out.println("  Test type:   " + bmResult.getBmType());
            
            if (bmResult.getNumberBmThreads() > 0) {
                System.out.println("  Threads:     " + bmResult.getNumberBmThreads());
            }
            if (bmResult.getWarmupTime() > 0) {
                System.out.println("  Warmup:      " + TestResult.millisAsSec(bmResult.getWarmupTime()));
            }
            System.out.println(
                    "  Iterations:  " + (miniter < maxiter ? "Auto run mode " + miniter + ".." + maxiter : (maxiter == -1 ? "infinite" : "" + maxiter)));
            System.out.println("  Run length:  " + bmResult.durDesc());
            ...
            if (bmResult.getBenchmarkForcegc() || bmResult.getBenchmarkDelay() > 0) {
                ...
                if (bmResult.getBenchmarkForcegc()) {
                    System.out.println("Requests gc before starting.");
                    System.gc();
                ...
                if (bmResult.getBenchmarkDelay() > 0) {
                    System.out.println("Pause for " + TestResult.millisAsSec(bmResult.getBenchmarkDelay()));
                ...
                if (!bmResult.getForcedIterationTime()) {
                    if (valid && itResult.getScore() < 5 / (((double) bmResult.getIterationTime()) / (1000 * 60))) {
                        ...
                        if (dur > bmResult.getIterationTime()) {
                            ...
                            System.out.println("Warning: Current run time for one iteration is too low.");
                            System.out.println("Increasing iteration time to " + TestResult.millisAsSec(bmResult.getIterationTime()));
                        }
                    }
                }
            }
                ...
                if (valid && iter + 1 <= maxiter) { // Skip pause after last
                    ...
                    if (bmResult.getBenchmarkForcegc() || bmResult.getBenchmarkDelay() > 0) {
                        ...
                        if (bmResult.getIterationForcegc()) {
                            System.out.println("Requests gc between iterations.");
                            System.gc();
                        ...
                        if (bmResult.getIterationDelay() > 0) {
                            System.out.println("Pause for " + TestResult.millisAsSec(bmResult.getIterationDelay()));
        ...
        if (!valid) {
            System.out.println("Errors in benchmark: " + bmResult.getLogicalName());
            for (Iterator ei = bmResult.getAllErrors("").iterator(); ei.hasNext();) {
                System.out.println("  " + (String) ei.next());
            }
        } else {
        ...
        if (!bmResult.getBmType().equals(SpecJVMBenchmark.FUNCTIONAL)) {
            System.out.println("Score on " + bmResult.getLogicalName() + ": " + bmResult.resultString());
        }
        System.out.println();
    ...
    private boolean runIteration(String runDesc, IterationResult itResult, BenchmarkResult bmResult) {
        ...
        try {       
            ...
            System.out.println();
            System.out.println(runDesc + " begins: " + new Date());
            System.out.flush();
        ...
        System.out.println(runDesc + " ends:   " + new Date());
        System.out.println(runDesc + " result: " + itResult.resultString(bmResult.getBmType()));
        for (Iterator<AnalyzerResult> iter = itResult.getAnalyzerSummaries(); iter.hasNext(); ) {
            System.out.println(runDesc + " analyzer result " + iter.next().getDescription());
        }
