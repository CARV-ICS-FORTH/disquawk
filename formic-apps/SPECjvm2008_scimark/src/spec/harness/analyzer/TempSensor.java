...
public class TempSensor {
    ...
    public TempSensor(String host, int port) {
        ...
        if (verbose) {
            System.out.println("Reader/writer set up\n");
        }
        ...
        try {
            ...
            if (verbose) {
                System.out.println("Meter response: " + meterResponse);
            }
            ...
            if (meterResponseParts.length == 12) {
                ...
                if (verbose) {
                    System.out.println("Temperature sensor type: " + meterResponseParts[0]);
                    System.out.println("Temperature sensor SPEC compliant: " + meterResponseParts[8]);
                    System.out.println("PTD version: " + meterResponseParts[9]);
                    System.out.println("PTD host OS: " + meterResponseParts[10]);
                    System.out.println("PTD mode: " + meterResponseParts[11]);
                }
            } else { // error in meter response
                System.out.println("Warning! Unexpected input from temperature sensor: " + meterResponse);
                System.out.println("Expected 12 tokens, separated with ','.");
            }
        ...
        if (verbose) {
            System.out.println("Meter connection successful!\n");
        }
    }
    ...
    public void startRun(int samples, int rate_ms, int rampup_ms) {
        ...
        writeMeter(msg);
        if (verbose) {
            System.out.println("Meter responds:" + readMeter());
        }
    }
    ...
    public HashMap<String, String> reportRun(String propPrefix) {
        ...
        for (int i = 0; i < 1; i++) {
            ...
            if (msg_parts.length == 7) {
                ...
                System.out.println("Warning! Unexpected input from temperature sensor: " + msg);
                System.out.println("Expected 7 tokens, separated with ','.");
