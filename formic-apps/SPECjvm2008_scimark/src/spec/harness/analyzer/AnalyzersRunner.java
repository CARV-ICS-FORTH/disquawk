...
public class AnalyzersRunner extends Thread {
    ...
    public void run() {
        ...
        while (cont && !isDone()) {
            try {
                ...
                cont = false;
                if (debug) {
                    System.out.println("[Analyzers] Interrupted to be closed down.");
                }
            }
        }
    ...
    private boolean executeAnalyzers() {
        ...
        if (debug) {
            System.out.println("[Analyzers] Executing analyzers.");
        }
    ...
    public static void setupAnalyzerClasses(String analyzers) throws ClassNotFoundException {
        ...
        if (debug) {
            System.out.println("[Analyzers] Setting up analyzer classes.");
        }
    ...
    public static void tearDownAnalyzerClasses() {
        ...
        if (debug) {
            System.out.println("[Analyzers] Tearing down analyzer classes.");
        }
    ...
    public boolean internalSetupAnalyzers() {
        ...
        if (debug) {
            System.out.println("[Analyzers] Setting up analyzers.");
        }
        ...
        for (int i = 0; i < analyzers.length; i++) {
            try {
                ...
                ret = false;
                System.out.println("Error instantiating analyzer " + analyzerClasses[i].getName() + ": " + e.getMessage());
                ...
                ret = false;
                System.out.println("Error setting up analyzer " + analyzerClasses[i].getName() + ": " + t.getMessage());
    ...
    public static void invokeStartMeasurementIntervalForAnalyzers() {
        ...
        if (debug) {
            System.out.println("[Analyzers] Invoking start measurement interval for analyzers.");
        }
        ...
        for (int i = 0; i < analyzerRunner.analyzers.length; i++) {
            try {
                ...
                System.out.println("Error invoking start measurement interval on analyzer "
    ...
    public static void invokeEndMeasurementIntervalForAnalyzers() {
        ...
        if (debug) {
            System.out.println("[Analyzers] Invoking end measurement interval for analyzers.");
        }
        ...
        for (int i = 0; i < analyzerRunner.analyzers.length; i++) {
            try {
                ...
                System.out.println("Error invoking end measurement interval on analyzer "
                        + analyzerClasses[i].getName() + ": "
                        + t.getMessage());
                t.printStackTrace(System.out);
            }
    ...
    private void internalTearDownAnalyzers() {
        ...
        if (debug) {
            System.out.println("[Analyzers] Tearing down analyzers.");
        }
        ...
        for (int i = 0; i < analyzers.length; i++) {
            try {
                ...
                System.out.println("Error setting up analyzer "
                        + analyzerClasses[i].getName() + ": "
                        + t.getMessage());
                t.printStackTrace(System.out);
            }
