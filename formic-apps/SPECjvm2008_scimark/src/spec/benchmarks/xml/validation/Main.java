...
public class Main extends XMLBenchmark {
    ...
    private void executeWorkload() throws
        ...
        for (int i = 0; i < XSD_NUMBER; i++) {
            System.out.println("Validating " + instanceNames[i]);
    ...
    private void validateSource(int loop, Source source, Validator schemaValidator) {
        ...
        try {
            ...
            if (loop == 0) {
                System.out.print("\tas " + source.getClass().getName());
                System.out.println(" succeeded. (correct result)");
            }
        } catch (SAXException e) {
            System.out.print("\tas " + source.getClass().getName());
            System.out.println(" failed. (Incorrect result)" + Arrays.toString(loops));
        //    e.printStackTrace(System.out);
        } catch (IOException e) {
            System.out.println("Unable to validate due to IOException.");
        }
    }
}
