...
public class Compiler {    
    ...
    public void compile(int compiles) {      
        ...
        for (int i = compiles - 1; i >=0; i--) {     
            ...
            if (r != 0) {
                spec.harness.System.out.println("ERROR: compiler exit code: "
                		+ Integer.toString(r));
                break;
            }
            if (skipVerify) {
            ...
            if (i == 0) {
            	...
            	spec.harness.System.out.println("Total checksum:" + checkSum);
            } else if (i == compiles - 1) {
            	checkSum = fileManager.getChecksum();
            } else {
            	if (checkSum != fileManager.getChecksum()) {
            		spec.harness.System.out.println("Total checksum on " 
