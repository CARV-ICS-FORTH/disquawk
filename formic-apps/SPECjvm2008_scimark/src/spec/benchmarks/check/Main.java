...
public class Main extends SpecJVMBenchmarkBase {
    ...
    public long runBenchmark() {
        ...
        try {
            ...
            for (int i = 0; i <= 10; i++)
                a[i] = i;
            spec.harness.System.out.println("Error: array bounds not checked");
        ...
        if (!caughtIndex) {
            spec.harness.System.out.println("1st bounds test error:\tindex exception not received");
        }
        if (!gotToFinally) {
            spec.harness.System.out.println("1st bounds test error:\tfinally clause not executed");
        }
        if (caughtIndex && gotToFinally) {
            spec.harness.System.out.println("1st bounds test:\tOK");
        }
        ...
        if (LoopBounds.gotError) {
            spec.harness.System.out.println("2nd bounds test:\tfailed");
        } else {
            spec.harness.System.out.println("2nd bounds test:\tOK");
        }
        
        if(!checkCompilerVersion()) {
            spec.harness.System.out.println("Compiler version test:\tfailed");
        ...
        if (horse.gotError) {
            spec.harness.System.out.println("PepTest failed");
        }
    ...
    private static void checkSubclassing() {
        ...
        Sub sub = new Sub(3);
        spec.harness.System.out.println(sup.getName() + ": " + sup.toString());
        spec.harness.System.out.println(sub.getName() + ": " + sub.toString());
        spec.harness.System.out.println("Super: prot=" + sup.getProtected() + ", priv=" + sup.getPrivate());
        spec.harness.System.out.println("Sub:  prot=" + sub.getProtected() + ", priv=" + sub.getPrivate());
    }
