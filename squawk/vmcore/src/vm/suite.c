                        ...
                        int     *hash)
{
    ...
    hasTypemap = ((attributes & com_sun_squawk_ObjectMemoryFile_ATTRIBUTE_TYPEMAP) != 0);
    if (((attributes & com_sun_squawk_ObjectMemoryFile_ATTRIBUTE_32BIT) != 0) == SQUAWK_64) {
        fatalVMError("word size in bootstrap suite is incorrect");
    }
    if (((attributes & com_sun_squawk_ObjectMemoryFile_ATTRIBUTE_BIGENDIAN) != 0) != PLATFORM_BIG_ENDIAN) {
