...
/*end[JAVA5SYNTAX]*/
	 ...
	 * a sequence of code that exposes some allocated memory before it is been
...
/*end[DEBUG_CODE_ENABLED]*/
	...
	private static Object allocate(int size, Object klass, int arrayLength) {
		Object oop =
			(excessiveGC && !VMThread.currentThread().isServiceThread())
			? null
			: allocatePrim(size, klass, arrayLength);
		if (oop == null) {
...
/*end[DEBUG_CODE_ENABLED]*/
		...
		Assert.that(VM.isHosted()
		            || !inRam(klass)
		            || Address.fromObject(klass).lo(Address.fromObject(oop)));
		return oop;
	}
...
/*end[DEBUG_CODE_ENABLED]*/
	...
	static Object newInstance(Klass klass) {
		Object oop =
			allocate((klass.getInstanceSize() * HDR.BYTES_PER_WORD) + HDR.basicHeaderSize, klass, -1);
