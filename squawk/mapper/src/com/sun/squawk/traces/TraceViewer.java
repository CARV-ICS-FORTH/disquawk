...
/*if[MICROBLAZE_BUILD]*/
		Matcher m = Pattern.compile("\\*TRACE\\*:\\*ROM\\*:(\\d+):(\\d+):\\*(\\d+)\\*").matcher(line);
		if (!m.matches()) {
			return null;
		}
...
/*end[MICROBLAZE_BUILD]*/
	 ...
	 * @return the matcher used for a successful match or null
	 */
	private Matcher matchesStackTraceRepetitionElement(String line) {
	...
	private ThreadSliceNode parseTraceLine(String line, int tracePosition, ThreadSliceNode currentThread, boolean showServiceThread) {
		...
		// Match *STACKTRACE*:"<n>
		if ((m = matchesStackTraceRepetitionElement(line)) != null) {
	...
	private void readTrace(File file, boolean showServiceThread) {
		...
		try {
            ...
            AddressRelocator relocator;
			do {
				line = in.readLine();
				relocator = createAddressRelocator(line);
			} while (relocator == null && line != null);

			if (relocator == null)
				throw new TraceParseException(line, "*TRACE*:*ROM*:(d+):(d+):*(d+)*");
...
/*end[ENABLE_DYNAMIC_CLASSLOADING]*/
		...
		for (Enumeration e = symbolsToLoad.elements(); e.hasMoreElements();) {
			...
			System.out.println("Loading "+symbolsFile);
